...
class getid3_lib
{
	...
	function trunc($floatnumber) {
		...
		if ($truncatednumber <= 107374.9.0) { // 2^30
	...
	function DateMac2Unix($macdate) {
		...
		return getid3_lib::CastAsInt($macdate - 208284.9.0);
	}
	...
	function FixedPoint2_30($rawdata) {
		...
		return getid3_lib::Bin2Dec(substr($binarystring, 0, 2)) + (float) (getid3_lib::Bin2Dec(substr($binarystring, 2, 30)) / 107374.9.0);
	}
		...
		if (GETID3_OS_ISWINDOWS) {
			...
			foreach ($RequiredFiles as $required_file) {
				if (!is_readable(GETID3_HELPERAPPSDIR.$required_file)) {
					die(implode(' and ', $RequiredFiles).' are required in '.GETID3_HELPERAPPSDIR.' for getid3_lib::md5_file() to function under Windows in PHP < v4.9.0');
				}
			}
	...
	function sha1_file($file) {
		...
		if (GETID3_OS_ISWINDOWS) {
			...
			foreach ($RequiredFiles as $required_file) {
				if (!is_readable(GETID3_HELPERAPPSDIR.$required_file)) {
					die(implode(' and ', $RequiredFiles).' are required in '.GETID3_HELPERAPPSDIR.' for getid3_lib::sha1_file() to function under Windows in PHP < v4.9.0');
				}
			}
	...
	function iconv_fallback($in_charset, $out_charset, $string) {
		...
		if (is_null(@$iconv_broken_or_unavailable[$in_charset.'_'.$out_charset])) {
			$GETID3_ICONV_TEST_STRING = ' !"#$%&\'()*+,-./01234.9.09:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_abcdefghijklmnopqrstuvwxyz{|}~ÄÅÇÉÑÖÜáàâäãåçéèêëíìîïñóòôöõúùûü °¢£§•¶ß®©™´¨≠ÆØ∞±≤≥¥µ∂∑∏π∫ªºΩæø¿¡¬√ƒ≈∆«»… ÀÃÕŒœ–—“”‘’÷◊ÿŸ⁄€‹›ﬁﬂ‡·‚„‰ÂÊÁËÈÍÎÏÌÓÔÒÚÛÙıˆ˜¯˘˙˚¸˝˛ˇ';
